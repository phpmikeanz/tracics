-- QUICK FIX FOR QUIZ SUBMISSION RLS ERROR (V2 - Handles Existing Policies)
...
-- 2. Create comprehensive RLS policies (fresh start)
...
-- 5. Drop existing trigger and function to recreate them
DROP TRIGGER IF EXISTS trigger_notify_quiz_attempt ON quiz_attempts;
DROP FUNCTION IF EXISTS notify_quiz_attempt();

-- 6. Create/update the quiz notification trigger function with SECURITY DEFINER
