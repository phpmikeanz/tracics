...
-- Quiz completion trigger - fires when status changes to 'completed'
...
CREATE TRIGGER trigger_notify_quiz_graded
    ...
    FOR EACH ROW
    WHEN (NEW.status = 'graded' AND (OLD.status IS NULL OR OLD.status != 'graded'))
...
BEGIN
    ...
    UNION ALL
    SELECT 
        'trigger_notify_quiz_completion'::TEXT,
        'quiz_attempts'::TEXT,
        CASE 
            WHEN EXISTS (
                SELECT 1 FROM information_schema.triggers 
                WHERE trigger_name = 'trigger_notify_quiz_completion'
            ) THEN '✅ Active'
            ELSE '❌ Missing'
        END,
    ...
    SELECT 
        ...
        CASE 
            WHEN EXISTS (
                SELECT 1 FROM information_schema.triggers 
                WHERE trigger_name = 'trigger_notify_quiz_graded'
            ) THEN '✅ Active'
            ELSE '❌ Missing'
        END,
